...
core::expected<u32, ShaderProg::error_type> compileShader(u32 type, std::string_view src) {
...
core::expected<ShaderProg, ShaderProg::error_type> ShaderProg::create(std::string_view vertexShaderSrc,
    ...
    auto res = compileShader(GL_VERTEX_SHADER, vertexShaderSrc);
    if (res.has_err()) return core::unexpected(core::move(res.err()));
    u32 vs = res.value();
    defer { glDeleteShader(vs); };

    auto res2 = compileShader(GL_FRAGMENT_SHADER, fragShaderSrc);
...
core::expected<i32, ShaderProg::error_type> ShaderProg::getAttribLocation(std::string_view name) {
