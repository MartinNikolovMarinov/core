...
using ParseError = CmdFlagParser::ParseError;
using MatchError = CmdFlagParser::MatchError;
using ParsedSymbol = CmdFlagParser::ParsedSymbol;
using FlagType = CmdFlagParser::FlagType;
using FlagData = CmdFlagParser::FlagData;

using ParsedSymbols = CmdFlagParser::ParsedSymbols;
using FlagDataMap = CmdFlagParser::FlagDataMap;
using FlagValidationFn = CmdFlagParser::FlagValidationFn;

void CmdFlagParser::allowUnknownFlags(bool allow) {
    m_allowUnknownFlags = allow;
}

core::expected<ParseError> CmdFlagParser::parse(addr_size argc, const char** argv) {
...
const ParsedSymbols& CmdFlagParser::parsedSymbols() const {
    return m_parsedSymbols;
}

StrView CmdFlagParser::programName() const {
    StrView ret = !m_parsedSymbols.empty() ? m_parsedSymbols[0].value.view() : sv();
    return ret;
}

core::expected<MatchError> CmdFlagParser::matchFlags() {
...
template <typename T>
void _insertFlag(FlagDataMap& flagData, T* out, StrView flagName, bool required, FlagValidationFn validation, FlagType type) {
    FlagData* existing = flagData.get(flagName);
    if (existing) {
    ...
    flagData.put(key, std::move(fd));
}

} // namespace

void CmdFlagParser::setFlagString(StrBuilder* out, StrView flagName, bool required, FlagValidationFn validation) {
    _insertFlag(this->m_flagData, out, flagName, required, validation, FlagType::String);
}

void CmdFlagParser::setFlagBool(bool* out, StrView flagName, bool required, FlagValidationFn validation) {
    _insertFlag(this->m_flagData, out, flagName, required, validation, FlagType::Bool);
}

void CmdFlagParser::setFlagInt32(i32* out, StrView flagName, bool required, FlagValidationFn validation) {
    _insertFlag(this->m_flagData, out, flagName, required, validation, FlagType::Int32);
}

void CmdFlagParser::setFlagInt64(i64* out, StrView flagName, bool required, FlagValidationFn validation) {
    _insertFlag(this->m_flagData, out, flagName, required, validation, FlagType::Int64);
}

void CmdFlagParser::setFlagUint32(u32* out, StrView flagName, bool required, FlagValidationFn validation) {
    _insertFlag(this->m_flagData, out, flagName, required, validation, FlagType::Uint32);
}

void CmdFlagParser::setFlagUint64(u64* out, StrView flagName, bool required, FlagValidationFn validation) {
    _insertFlag(this->m_flagData, out, flagName, required, validation, FlagType::Uint64);
}

void CmdFlagParser::setFlagFloat32(f32* out, StrView flagName, bool required, FlagValidationFn validation) {
    _insertFlag(this->m_flagData, out, flagName, required, validation, FlagType::Float32);
}

void CmdFlagParser::setFlagFloat64(f64* out, StrView flagName, bool required, FlagValidationFn validation) {
    _insertFlag(this->m_flagData, out, flagName, required, validation, FlagType::Float64);
}

void CmdFlagParser::alias(StrView flagName, StrView aliasName) {
