...
i32 start2ThreadsAndJoinThemTest() {
    ...
    while (iterations--) {
        ...
        Args* t1Args = reinterpret_cast<Args*>(actx.alloc(1, sizeof(Args)));
        CT_CHECK(t1Args != nullptr);
        *t1Args = {&t1Done};
        defer { actx.free(t1Args, 1, sizeof(Args)); };
        Args* t2Args = reinterpret_cast<Args*>(actx.alloc(1, sizeof(Args)));
        CT_CHECK(t2Args != nullptr);
        *t2Args = {&t2Done};
        defer { actx.free(t2Args, 1, sizeof(Args)); };
