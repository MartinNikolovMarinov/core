...
constexpr i32 expectedBasicCaseTest() {
    ...
    CT_CHECK(e2.hasErr());
    CT_CHECK(core::memcmp(e2.err(), core::cstrLen(e2.err()), "bad", core::cstrLen(e2.err())) == 0);
...
constexpr i32 expectedUsedInAFunctionTest() {
    ...
    CT_CHECK(f(-1).hasErr());
    CT_CHECK(core::memcmp(f(-1).err(), core::cstrLen(f(-1).err()), errMsg1, core::cstrLen(errMsg1)) == 0);
