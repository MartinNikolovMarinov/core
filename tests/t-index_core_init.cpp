...
static core::StdStatsAllocator g_defaultAllocator;

static core::StdAllocator g_stdAllocator;
...
void coreInit() {
    core::initProgramCtx(assertHandler, nullptr, core::createAllocatorCtx(&g_defaultAllocator));
...
i32 runAllTests() {
    ...
    sInfo.name = FN_NAME_TO_CPTR(runStackTestsSuite);
    if (runTestSuite(sInfo, runStackTestsSuite) != 0) { ret = -1; }
    ...
    // NOTE: Skipping dut to slow execution time:
